#!/usr/bin/env python3
"""
ongc_engine.py — the ONGC dividend paper bot daemon. PHASE: PAPER (real feed or synthetic).

Sibling of nifty_engine.py / spx_engine.py, and deliberately near-identical: same single-writer
EngineState, same reuse of the config-driven apex_api shim (here on :8088), same pessimistic
both-sides-fee paper fills, same dry_run refusal. It reuses div_strategy.DividendStrategy,
which is instrument-agnostic (config["div_strategy"]["ticker"]), so ongc_engine and itc_engine
differ only by config. One strategy, two instruments, no dialect drift.

The differences from nifty_engine are the ledger (ongc_store), the exchange label ("ongc"), the
instrument (ONGC, an NSE single stock rather than an ETF) — and one genuinely new mechanic:

DIVIDEND ACCRUAL AS CASH. ONGC is held for a ~7.4% yield, and a bot that only ever books price
...
DEFAULT_CONFIG = os.path.join(BASE, "config_ongc.json")
STATE_FILE = os.path.join(BASE, "ongc_state.json")   # git-ignored by the *_state.json rule
...
def _row_to_trade(row):
    ...
    return {
        ...
        "exchange": row.get("exchange", "ongc"), "strategy": row.get("strategy"),
...
def _merge(base, over):
    """Recursive dict merge, later wins — so config_ongc.secret.json can slot
...
class EngineState:
    ...
    pointed at ONGC.NS via config, plus the dividend-accrual bookkeeping."""
    ...
    def __init__(self, config):
        ...
        self.next_trade_id = ongc_store.max_trade_id() + 1
        for row in ongc_store.load_open_trades():
            try:
                self.positions.append(_row_to_trade(row))
            except Exception as e:
                print(f"ongc_engine: skip unrecoverable open row {row.get('id')}: {e}", flush=True)
    ...
    def _open(self, pair, side="long", enter_tag=None):
        ...
        trade = {
            ...
            "exchange": "ongc", "strategy": "OngcDividendDMA", "enter_tag": enter_tag,
        ...
        trade_id = ongc_store.open_trade(trade)
        trade["id"] = trade_id
    ...
    def accrue_dividends(self, today=None):
        ...
        `today` defaults to the UTC date; the explicit param exists so ongc_paper_test.py can
        ...
        try:
            ...
            with self._lock:
                for t in list(self.positions):
                    ...
                    credited += cash
                    print(f"ongc_engine: ex-date dividend credited ₹{cash:.2f} "
                          f"({shares:.4f} shares × ₹{rate:.2f})", flush=True)
            return credited
        except Exception as e:
            # A broken ex_dates list must cost one missed credit, never the daemon loop.
            print(f"ongc_engine: dividend accrual skipped: {type(e).__name__}: {e}", flush=True)
            return 0.0
    ...
    def cycle_strategy(self):
        ...
        if action == "enter":
            self._open(sig.get("pair", "ONGC/INR"),
...
def main():
    ap = argparse.ArgumentParser(description="ONGC dividend paper bot daemon")
    ...
    try:
        ...
        print(f"ongc_engine: cannot load {args.config}: {type(e).__name__}: {e}", flush=True)
        return 1

    # No broker order path exists in this file; a live config would mean unmanaged capital.
    if not cfg.get("dry_run", True):
        print("ongc_engine: REFUSING to start — dry_run is false and the paper engine has "
    ...
    port = int((cfg.get("api_server") or {}).get("listen_port", 8088))
    ...
    try:
        ...
        print(f"ongc_engine: REST shim failed to start on :{port}: {type(e).__name__}: {e}", flush=True)
        return 1

    mode = "PAPER (synthetic)" if cfg.get("synthetic") else "PAPER (real ONGC.NS feed)"
    print(f"ongc_engine: up — dry_run wallet ₹{state.balance:.2f}, shim on :{port}, "
    ...
    while True:
        try:
            ...
            if now - last_beat >= HEARTBEAT_EVERY:
                last_beat = now
                snap = state.snapshot()
                print(f"ongc_engine: heartbeat cycles={snap['cycles']} "
            ...
            print(f"ongc_engine: cycle error: {type(e).__name__}: {e}", flush=True)
        time.sleep(throttle)
