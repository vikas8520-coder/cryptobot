#!/usr/bin/env python3
"""
btc_paper_test.py — BTC paper-bot proof harness (one-shot, not auto-run).

Same role as nifty_paper_test.py / spx_paper_test.py: drives the paper trade lifecycle
deterministically and asserts the whole chain — strategy signal -> paper fill -> sqlite
row -> stats_lib query. Forces config["synthetic"]=true so the run is reproducible with
NO network (a real BTC-USD pull would make the assertion numbers move every run), then
verifies the ledger stats_lib will read.
...
Checks:
  ...
  (c) tradesv3_btc.sqlite carries the CLOSED row with the columns stats_lib reads
...
def main():
    ...
    if cp is None or cpa is None:
        ...
        return 1
    # Spot crypto pays nothing for holding — a non-zero accrual here would be invented P&L.
    if row[6]:
        print(f"FAIL (c): funding_fees={row[6]} on a spot-crypto paper trade — must be 0")
        return 1
    ...
    print("\nPASS: strategy -> paper fill -> ledger -> stats_lib chain verified "
          f"({n} closed trade, net {net_abs:+.4f} USD, fees {fees:.4f})")
    return 0
