#!/usr/bin/env python3
"""
ongc_store.py — the `tradesv3_ongc.sqlite` writer. The ONGC dividend paper bot's ledger.

Identical in structure and discipline to nifty_store.py / spx_store.py / apex_store.py (see
apex_store.py's header for the full rationale: freqtrade column dialect is the contract,
booleans store as 0/1, atomic private-copy + os.replace writes, journal_mode DELETE). Only
three things differ from nifty_store: the DB filename, the default exchange tag ("ongc"), and
the instrument (ONGC, an NSE single stock priced in INR, held for its ~7.4% dividend yield).

ONE LEDGER PER BOT, always — a shared file would make stats_lib's per-bot queries and the
dashboard's per-bot equity lines silently blend two strategies together.

Dividend income is deliberately NOT a row in here. The engine credits it as cash (see
ongc_engine.accrue_dividends); folding a payout into close_profit_abs would make the SMA
strategy's win rate report the dividend as alpha.
"""
import os
import shutil
import sqlite3
...
DB_PATH = os.path.join(BASE, "tradesv3_ongc.sqlite")   # git-ignored by the *.sqlite rule
BUSY_TIMEOUT = 5
...
DEFAULTS = {
    "exchange": "ongc", "base_currency": "ONGC", "stake_currency": "INR",
    ...
    "strategy": "OngcDividendDMA", "enter_tag": None, "timeframe": 60,
...
if __name__ == "__main__":
    ensure_db()
    print(f"ongc_store: {DB_PATH}")
    print(f"ongc_store: {len(load_open_trades())} open, max id {max_trade_id()}")
