#!/usr/bin/env python3
"""
spx_paper_test.py — S&P 500 paper-bot proof harness (one-shot, not auto-run).

Same role as apex_paper_test.py: drives the paper trade lifecycle deterministically and
asserts the whole chain — strategy signal -> paper fill -> sqlite row -> stats_lib query.
Forces config["synthetic"]=true so the run is byte-for-byte reproducible with NO network
(yfinance never called), then verifies the ledger stats_lib will read.
...
Checks:
  ...
  (c) tradesv3_spx.sqlite carries the CLOSED row with the columns stats_lib reads
...
CFG = os.path.join(BASE, "config_spx.json")
DB = spx_store.DB_PATH


def main():
    spx_store.ensure_db()
    cfg = spx_engine.load_config(CFG)
    cfg["synthetic"] = True   # deterministic, no network — this is a plumbing test
    state = spx_engine.EngineState(cfg)

    # --- (a) open a trade ---
    t = state._open("SPY/USD", side="long", enter_tag="test")
    ...
    if open_id not in [p["trade_id"] for p in state.snapshot()["positions"]]:
        ...
        return 1
    open_rows = spx_store.load_open_trades()
    ...
    if not ok:
        ...
        return 1
    # Our specific trade must be gone from open positions. (The ledger accumulates across
    # runs, so OTHER leftover open rows from prior runs may remain — that's expected; we
    # assert on our own id, not a global-empty book, same as check (d).)
