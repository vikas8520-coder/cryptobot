#!/usr/bin/env python3
"""
btc_engine.py — the BTC paper bot daemon. PHASE: PAPER (real feed or synthetic).

Sibling of nifty_engine.py / spx_engine.py, and deliberately near-identical: same
single-writer EngineState, same reuse of the config-driven apex_api shim (here on :8091),
same pessimistic both-sides-fee paper fills, same dry_run refusal. It reuses
spx_strategy.SpxStrategy VERBATIM — that strategy is instrument-agnostic
(config["spx_strategy"]["ticker"]), so this bot just points it at BTC-USD instead of SPY.
One strategy, three markets, no dialect drift.

The only real differences from nifty_engine: the ledger (btc_store), the currency (USD),
the exchange label ("btc"), and the instrument (BTC-USD via yfinance — NOT ccxt, so the
crypto bot rides the same proven feed path as the equity paper bots instead of adding a
second market-data stack to maintain).

Two consequences of the instrument that are NOT cosmetic:
  - NO dividend/funding accrual. Spot crypto pays nothing for holding; a bot that credited
    anything here would be inventing P&L (contrast ongc/itc, which accrue real dividends).
  - The market never closes. spx_strategy's stale_secs idle-guard exists for the equity
    bots' overnight gap; on a 24/7 instrument it should only ever fire when the FEED is
    broken — which is exactly the signal we want it to carry here.

PAPER only, same as SPX/Nifty: no broker order path exists in this file; it refuses to
start unless dry_run is true. Going live is a separate build (exchange + keys), not a flip.
"""
import argparse
import json
import os
...
DEFAULT_CONFIG = os.path.join(BASE, "config_btc.json")
STATE_FILE = os.path.join(BASE, "btc_state.json")   # git-ignored by the *_state.json rule

DEFAULT_THROTTLE = 5
HEARTBEAT_EVERY = 300
DEFAULT_FEED_REFRESH = 300
DEFAULT_PORT = 8091        # 8090 is the dashboard — do NOT reuse it (bind collision)
...
def _row_to_trade(row):
    ...
    return {
        ...
        "exchange": row.get("exchange", "btc"), "strategy": row.get("strategy"),
...
def _merge(base, over):
    """Recursive dict merge, later wins — so config_btc.secret.json can slot
...
class EngineState:
    ...
    by the uvicorn thread. Mirrors nifty_engine.EngineState; strategy is SpxStrategy
    pointed at BTC-USD via config."""

    def __init__(self, config):
        self.config = config
        self.starting_capital = float(config.get("dry_run_wallet", 500))
        ...
        self.next_trade_id = btc_store.max_trade_id() + 1
        for row in btc_store.load_open_trades():
            try:
                self.positions.append(_row_to_trade(row))
            except Exception as e:
                print(f"btc_engine: skip unrecoverable open row {row.get('id')}: {e}", flush=True)
    ...
    def _open(self, pair, side="long", enter_tag=None):
        ...
        trade = {
            ...
            "exchange": "btc", "strategy": "BtcSmaCross", "enter_tag": enter_tag,
        ...
        trade_id = btc_store.open_trade(trade)
        trade["id"] = trade_id
    ...
    def _check_stops(self):
        ...
        WHY: the engine previously exited ONLY on the SMA-cross-down signal, so an
        adverse move with no cross bled unbounded — the desk was -18.6% largely from
        this. Caps per-trade downside regardless of strategy. audit 2026-07-22.
        """
    ...
    def cycle_strategy(self):
        ...
        if action == "enter":
            self._open(sig.get("pair", "BTC/USD"),
...
def main():
    ap = argparse.ArgumentParser(description="BTC paper bot daemon")
    ...
    try:
        ...
        print(f"btc_engine: cannot load {args.config}: {type(e).__name__}: {e}", flush=True)
        return 1

    # No broker order path exists in this file; a live config would mean unmanaged capital.
    if not cfg.get("dry_run", True):
        print("btc_engine: REFUSING to start — dry_run is false and the paper engine has "
              "no broker order path. Going live is a separate build (exchange + keys), not "
              "a config flip.", flush=True)
        return 1
    ...
    port = int((cfg.get("api_server") or {}).get("listen_port", DEFAULT_PORT))
    ...
    try:
        ...
        print(f"btc_engine: REST shim failed to start on :{port}: {type(e).__name__}: {e}", flush=True)
        return 1

    mode = "PAPER (synthetic)" if cfg.get("synthetic") else "PAPER (real BTC-USD feed)"
    print(f"btc_engine: up — dry_run wallet ${state.balance:.2f}, shim on :{port}, "
    ...
    while True:
        try:
            ...
            if now - last_beat >= HEARTBEAT_EVERY:
                last_beat = now
                snap = state.snapshot()
                print(f"btc_engine: heartbeat cycles={snap['cycles']} "
                      f"balance={snap['balance']:.2f} open={len(snap['positions'])} "
                      f"paused={snap['paused']}", flush=True)
                heartbeat_file(state)
        except Exception as e:
            print(f"btc_engine: cycle error: {type(e).__name__}: {e}", flush=True)
        time.sleep(throttle)
