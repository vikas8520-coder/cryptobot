...
- **Auth is uniform:** every poller does `HTTPBasicAuth("freqtrader", api_pw(port))`
  ...
  `f"pass{port}"` (`local_secrets.py:18-19`).
...
|---|---|---|
| `local_secrets.py:9-15` | ports 8080-8084 | add `8085: "pass8085",   # apex (DEX)` |
...
- **Verify:** `curl -u freqtrader:pass8085 127.0.0.1:8085/api/v1/{ping,status,balance,profit,show_config,whitelist,daily?timescale=30}`
...
machine currently stores localhost passwords as `pass8080`-style literals
