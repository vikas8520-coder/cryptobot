#!/usr/bin/env python3
"""
btc_store.py — the `tradesv3_btc.sqlite` writer. The BTC paper bot's ledger.

Identical in structure and discipline to nifty_store.py / spx_store.py / apex_store.py
(see apex_store.py's header for the full rationale: freqtrade column dialect is the
contract, booleans store as 0/1, atomic private-copy + os.replace writes, journal_mode
DELETE). Only three things differ from nifty_store: the DB filename, the default exchange
tag ("btc"), and the stake currency ("USD" — BTC-USD spot off the yfinance feed).

NOTE the deliberate omission vs the dividend stores (ongc/itc): there is no dividend or
funding accrual column path here. Spot crypto pays nothing for holding, so a bot that
credited anything would be inventing P&L.
"""
import os
import shutil
import sqlite3
...
DB_PATH = os.path.join(BASE, "tradesv3_btc.sqlite")   # git-ignored by the *.sqlite rule
BUSY_TIMEOUT = 5
...
DEFAULTS = {
    "exchange": "btc", "base_currency": "BTC", "stake_currency": "USD",
    ...
    "strategy": "BtcSmaCross", "enter_tag": None, "timeframe": 60,
...
if __name__ == "__main__":
    ensure_db()
    print(f"btc_store: {DB_PATH}")
    print(f"btc_store: {len(load_open_trades())} open, max id {max_trade_id()}")
