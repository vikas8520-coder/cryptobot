#!/usr/bin/env python3
"""closing_hour_optimize.py — Find the best target/stop for the 14:00-15:00 closing hour.

The opening hour analysis showed wider target + tighter stop works because
opening volatility creates big directional moves. The closing hour may have
a different character — let's test it.
"""
import os
import sys
...
def run_closing_sweep():
    ...
    # Collect closing-hour signals (14:00-15:00)
    signals = []
    for i in range(EMA_PERIOD + 1, n - 1):
        t = pd.Timestamp(df["Date"].iloc[i]).time()
        if t < time(14, 0) or t > time(14, 59):
            continue
    ...
    print(f"\n{'='*80}")
    print(f"CLOSING HOUR (14:00-15:00) TARGET/STOP OPTIMIZATION")
    print(f"Signals found: {len(signals)}")
    print(f"{'='*80}\n")

    # Note: closing hour has fewer bars until EOD (~12-24 5m bars left)
    avg_bars = sum(len(s["forward_moves"]) for s in signals) / len(signals) if signals else 0
    print(f"Average bars until EOD: {avg_bars:.0f} (~{avg_bars*5:.0f} minutes)")
    print()
    ...
    # Last 30 minutes (14:30-15:00) — the final push
    print(f"\n{'='*80}")
    print("LAST 30 MINUTES ONLY (14:30-15:00) — closing bell push")
    print(f"{'='*80}\n")

    late_signals = [s for s in signals if s["time"] >= "14:30:00"]
    print(f"Signals in 14:30-15:00: {len(late_signals)}")
    avg_bars_late = sum(len(s["forward_moves"]) for s in late_signals) / len(late_signals) if late_signals else 0
    print(f"Average bars until EOD: {avg_bars_late:.0f} (~{avg_bars_late*5:.0f} minutes)")

    if late_signals:
        print()
        for target_pct, stop_pct in [(0.02, 0.02), (0.03, 0.02), (0.03, 0.03), (0.04, 0.03), (0.05, 0.03)]:
            results = []
