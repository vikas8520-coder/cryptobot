#!/usr/bin/env python3
"""
itc_store.py — the `tradesv3_itc.sqlite` writer. The ITC dividend paper bot's ledger.
...
three things differ from nifty_store: the DB filename, the default exchange tag ("itc"), and
the instrument (ITC, an NSE single stock priced in INR, held for its ~5.7% dividend yield).
...
itc_engine.accrue_dividends); folding a payout into close_profit_abs would make the SMA
...
DB_PATH = os.path.join(BASE, "tradesv3_itc.sqlite")   # git-ignored by the *.sqlite rule
BUSY_TIMEOUT = 5
...
DEFAULTS = {
    "exchange": "itc", "base_currency": "ITC", "stake_currency": "INR",
    ...
    "strategy": "ItcDividendDMA", "enter_tag": None, "timeframe": 60,
...
if __name__ == "__main__":
    ensure_db()
    print(f"itc_store: {DB_PATH}")
    print(f"itc_store: {len(load_open_trades())} open, max id {max_trade_id()}")
